...
class CalciatoreTest {
    ...
    @Test
    void setContrattoAltroCalciatore() {
        Calciatore c1 = new Calciatore(1, "Mario", "Rossi",
                LocalDate.of(1995, 1, 1), "Italia", 75, 180, ruoliBase);

        Calciatore c2 = new Calciatore(2, "Cristiano", "Ronaldo",
                LocalDate.of(1995, 2, 5), "Portogallo", 83, 187, ruoliBase);

        when(contrattoMock.getCalciatore()).thenReturn(c2);
        assertThrows(IllegalArgumentException.class, () -> {
            c1.setContrattoAttuale(contrattoMock);
        });
    }

    @Test
    void setContrattoValido() {
        Calciatore c1 = new Calciatore(1, "Mario", "Rossi",
                LocalDate.of(1995, 1, 1), "Italia", 75, 180, ruoliBase);
        when(contrattoMock.getCalciatore()).thenReturn(c1);

        c1.setContrattoAttuale(contrattoMock);

        assertEquals(c1.getContrattoAttuale(), contrattoMock);
    }

    @Test
    void isSvincolatoQuandoContrattoNull() {
        Calciatore c = new Calciatore(1, "Mario", "Rossi", LocalDate.of(1995, 1, 1), "Italia", 75, 180, ruoliBase);
        c.setContrattoAttuale(null);

        assertTrue(c.isSvincolato());
    }

    @Test
    void isSvincolatoQuandoContrattoScaduto() {
        Calciatore c = new Calciatore(1, "Mario", "Rossi", LocalDate.of(1995, 1, 1), "Italia", 75, 180, ruoliBase);

        when(contrattoMock.getCalciatore()).thenReturn(c); // Per passare il controllo del setter
        when(contrattoMock.isScaduto()).thenReturn(true);

        c.setContrattoAttuale(contrattoMock);

        assertTrue(c.isSvincolato());
    }

    @Test
    void isSvincolatoQuandoContrattoAttivo() {
        // Setup
        Calciatore c = new Calciatore(1, "Mario", "Rossi", LocalDate.of(1995, 1, 1), "Italia", 75, 180, ruoliBase);

        when(contrattoMock.getCalciatore()).thenReturn(c);
        when(contrattoMock.isScaduto()).thenReturn(false);

        c.setContrattoAttuale(contrattoMock);

        assertFalse(c.isSvincolato());
    }
}